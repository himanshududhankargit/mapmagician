// maps1-newregions.js — the "new regions" bottom sheet, ported from the Android app's
...
//
// LAZY-LOADED: maps1-app.js injects this script only when there is genuinely
// something to announce, so nothing here runs at page start. By the time it executes,
// the host globals (map, goToLocation, mmAnalytics) all exist — maps1-app.js is a
...
// and a diff sits exactly on the boundary between two published versions. maps1-app.js
...
(function () {
    ...
    // ---------------------------------------------------------------- constants
    var PENDING_KEY = 'mm_new_regions_pending';   // must match maps1-app.js
    ...
    // Injected rather than shipped in maps1.html so the whole feature — markup, style
    ...
    function navigate(row) {
        ...
        // search-result picker uses; see invokeSearchPick in maps1-app.js.
    ...
    window.mmNewRegions = {
        // Called by maps1-app.js when a diffed layer has been refetched after a
        ...
        // publish. Call it from the console any time, or let maps1-app.js's
        ...
        demo: function () {
            ...
            console.warn('[newregions] PREVIEW MODE — these regions are planted, not a ' +
                         'real publish. Turn off NR_DEMO_ON_LOAD in maps1-app.js before ' +
                         'promoting.');
